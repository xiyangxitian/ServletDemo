...
/*        Integer[] a = new Integer[]{1,2,3,4,5,6};
        ...
        Object[] objects = sumAll(a, 23861.2d);
        if(objects == null){//数太大 太多的时候是不行的。  超过选择7个就不行了。
            System.out.println("没有找到符合的！");
        }else{
    ...
    public static Object[] sumAll(Double[] obj, double xiangyao) throws Exception {
        for(int i=1;i<obj.length;i++){
            System.out.println("计算选出"+i+"个的时候");
            CombineAlgorithm ca = new CombineAlgorithm(obj, i);
            Object[][] result= ca.getResutl();
            ...
            for(int j=0;j<result.length-1;j++){
                ...
                for(Object o : objects){
                    sumD += Double.parseDouble(o.toString());
                }
                if(sumD == xiangyao){
//                    System.out.println(Arrays.toString(objects));
//                    break;
